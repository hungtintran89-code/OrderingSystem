...
export const StaffTableMap: React.FC = () => {
  ...
  const handleConfirmCompletePayment = async (tableId: string) => {
    try {
      const updated = await updateTableStatusApi(tableId, 'EMPTY');
      setTables((prev) =>
        prev.map((t) =>
          t.id === tableId ? { ...updated, currentOrderCode: undefined, totalAmount: undefined, occupiedMinutes: undefined } : t
        )
      );
      setIsCheckoutModalOpen(false);
      message.success('Đã hoàn tất thanh toán & giải phóng bàn!');
    } catch (err) {
  ...
  const handleGenerateVietQR = async () => {
    ...
    try {
      ...
      const totalAmt = selectedCheckoutTable.totalAmount || 340000;
  ...
  // SIMULATE PAYMENT RESULT (Thành công / Thất bại) - ĐƠN GIẢN KHÔNG DÙNG HIỆU ỨNG THỪA
    ...
    if (status === 'SUCCESS') {
      ...
      setTimeout(() => {
        if (selectedCheckoutTable) {
          handleConfirmCompletePayment(selectedCheckoutTable.id);
        }
      }, 1500);
    } else {
  ...
  // Get orders list for a table
  const getOrdersForTable = (tableNumber: string): OrderedItem[] => {
    return (
      mockTableOrders[tableNumber] || [
  ...
  return (
    <div className="space-y-4 font-sans">
      ...
      {/* STATE 1: NORMAL DATA INTERACTIVE TABLE GRID WITH 2 BUTTONS AT BOTTOM */}
      {!loading && !error && (
              ...
              >
                ...
                <div>
                  <div className="flex items-center justify-between">
                    <span className="font-extrabold text-base sm:text-lg">Bàn {table.tableNumber}</span>
      ...
      <Modal
        title={
          <div className="flex items-center gap-2">
            ...
            <span>📋 Chi Tiết Các Món Đã Đặt - Bàn {selectedDetailTable?.tableNumber}</span>
          </div>
        }
        ...
        {selectedDetailTable && (
          <div className="space-y-4 pt-2 text-xs font-sans">
            <div className="bg-slate-50 p-3 rounded-lg border border-slate-200 flex justify-between items-center">
              <div>
                <p className="font-bold text-slate-900 text-sm">Bàn {selectedDetailTable.tableNumber} • {selectedDetailTable.zone}</p>
            ...
            <div className="p-3.5 bg-orange-50 border border-orange-200 rounded-xl flex items-center justify-between">
              ...
              <span className="font-black text-xl text-orange-600">
                {formatVND(selectedDetailTable.totalAmount || 340000)}
              </span>
            </div>
      ...
      {/* 2. MODAL THANH TOÁN (CÓ NÚT "TẠO MÃ QR THANH TOÁN" RÕ RÀNG - KHÔNG TỰ SINH QR KHI BẤM CHỌN TAB) */}
      <Modal
        title={
          <div className="flex items-center gap-2">
            <CreditCard className="w-5 h-5 text-emerald-600" />
            <span>💳 Thanh Toán Hóa Đơn - Bàn {selectedCheckoutTable?.tableNumber}</span>
          </div>
        }
        ...
        {selectedCheckoutTable && (
          <div className="space-y-4 pt-2 text-xs font-sans">
            ...
            <div className="bg-slate-50 p-3 rounded-xl border border-slate-200 space-y-2">
              ...
              <div className="pt-2 border-t border-slate-200 flex items-center justify-between">
                ...
                <span className="font-black text-lg text-emerald-600">
                  {formatVND(selectedCheckoutTable.totalAmount || 340000)}
                </span>
              </div>
            </div>
            ...
            <div className="space-y-3">
              ...
              {paymentMethodTab === 'CASH' && (
                <div className="bg-white p-4 rounded-xl border border-slate-200 space-y-4 shadow-2xs">
                  ...
                  <div className="flex items-center gap-2 flex-wrap">
                    ...
                    {[
                      selectedCheckoutTable.totalAmount || 340000,
                      500000,
                      1000000,
                    ].map((amt) => (
                      <button
                  ...
                  {cashReceived !== '' && (
                    <div className="p-3 bg-emerald-50 border border-emerald-200 rounded-lg flex justify-between items-center">
                      ...
                      <span className="font-black text-base text-emerald-700">
                        {Number(cashReceived) >= (selectedCheckoutTable.totalAmount || 340000)
                          ? formatVND(Number(cashReceived) - (selectedCheckoutTable.totalAmount || 340000))
                  ...
                  <button
                    onClick={() => handleConfirmCompletePayment(selectedCheckoutTable.id)}
              ...
              {/* VIEW 2: THANH TOÁN MÃ QR (CHỈ TẠO QR KHI NGƯỜI DÙNG BẤM NÚT "TẠO MÃ QR THANH TOÁN") */}
              {paymentMethodTab === 'QR' && (
                <div className="bg-white p-4 rounded-xl border border-slate-200 space-y-4 text-center shadow-2xs">
                  ...
                  {isGeneratingQr && (
                    <div className="py-8 space-y-3">
                      ...
                      <p className="font-semibold text-slate-700">Đang tạo mã QR thanh toán VietQR cho Bàn {selectedCheckoutTable.tableNumber}...</p>
                    </div>
                  )}
                    ...
                    <div className="py-6 space-y-3 bg-slate-50 rounded-xl border border-slate-200">
                      ...
                      <div>
                        ...
                        <p className="text-slate-500 text-xs mt-1 max-w-sm mx-auto">
                          ...
                          <strong className="text-slate-900">{formatVND(selectedCheckoutTable.totalAmount || 340000)}</strong> của bàn.
                        </p>
                      </div>
                      ...
                      {qrPaymentStatus === 'PENDING' && (
                        <div className="bg-slate-50 p-4 rounded-xl border border-slate-200 space-y-3 inline-block w-full max-w-sm">
                          <div className="flex items-center justify-center gap-2 text-slate-800 font-bold text-xs border-b border-slate-200 pb-2">
                            ...
                            <span>NHÀ HÀNG F&B DINE-IN • PAYOS VIETQR</span>
                          </div>
                          ...
                          <div className="space-y-1 text-[11px] text-slate-600">
                            <p className="font-mono text-[10px] text-slate-400 truncate px-2">Link: {checkoutUrl}</p>
                            <p className="font-bold text-slate-900">
                              Số tiền: <span className="text-emerald-600">{formatVND(selectedCheckoutTable.totalAmount || 340000)}</span>
                            </p>
                          </div>
                          ...
                          {/* NÚT GIẢ LẬP KẾT QUẢ & NÚT HỦY GIAO DỊCH QR */}
                          <div className="pt-2 border-t border-slate-200 space-y-2">
                            <div className="flex justify-center gap-2">
                              <button
                                type="button"
                                onClick={() => handleSimulateQrResult('SUCCESS')}
                                className="px-2.5 py-1 rounded bg-emerald-100 hover:bg-emerald-200 text-emerald-800 text-[10px] font-bold cursor-pointer"
                              >
                                [Giả Lập Thành Công]
                              </button>
                              <button
                                ...
                                className="px-2.5 py-1 rounded bg-red-100 hover:bg-red-200 text-red-800 text-[10px] font-bold cursor-pointer"
                              >
                                [Giả Lập Thất Bại]
                              </button>
                            </div>
